import { verifyCodeForgotPassword } from '@/lib/services/auth.service';
import React, { useEffect, useRef, useState } from 'react';
import { useLocation, useNavigate } from 'react-router-dom';
import BackButton from '../vault/components/BackButton';

const EnterPinForgotPassword = () => {
  ...
  const handleVerifyPin = async (pinCode: string) => {
    try {
      ...
      await verifyCodeForgotPassword({ code: pinCode, email });
      // Navigate to reset-password with email and code as query params
      navigate(`/reset-password?email=${encodeURIComponent(email)}&code=${pinCode}`);
    } catch (error: any) {
    ...
    <div className="bg-white h-screen flex flex-col px-5 w-[65vh]">
      ...
      <div className="bg-white p-2 rounded-lg">
        ...
        <p className="text-center mt-6 text-gray-600">
          Quay lại{' '}
          <button className="text-blue-500 underline" onClick={() => navigate('/forgot-password')}>
            Nhập email
          </button>
        </p>
      </div>
...
export default EnterPinForgotPassword;
