import { turnOffSecurityApi, turnOnSecurityApi } from '@/lib/services/user.service';
...
const TurnOnSecurity = () => {
  ...
  useEffect(() => {
    ...
    actionRef.current = actionParam;
    setRedirectUrl(redirect);
    chrome.runtime.sendMessage({
      action: 'LOG',
      message: `[EnterPin] Redirect URL: ${redirect}`,
    });
  ...
  const handleVerifyPin = async (pinCode: string) => {
    try {
      ...
      if (actionRef.current === 'turn-on') {
        await turnOnSecurityApi(pinCode);
      } else {
        await turnOffSecurityApi(pinCode);
      }
      if (redirectUrl) {
